...
import { dispatchCampaignSignal } from '../../workflow/campaignGenerationWorkflow'
...
export function createCampaignRouter(workflowClient: WorkflowClient): Router {
    ...
    router.post('/:id/dispatch', async (req: Request<{ id: string }>, res: Response): Promise<void> => {
        try {
            const { id } = req.params
            const { segmentName, subject, bodyHtml } = req.body

            if (!segmentName || !subject || !bodyHtml) {
                res.status(400).json({ error: 'Missing required fields: segmentName, subject, bodyHtml' })
                return
            }

            try {
                const handle = workflowClient.getHandle(id)
                await handle.signal(dispatchCampaignSignal, segmentName, subject, bodyHtml)
                res.status(200).json({ success: true })
            } catch (error) {
                if (error instanceof WorkflowNotFoundError) {
                    res.status(404).json({ error: 'Campaign not found' })
